...
export default function ChatPage() {
  ...
  const fetchPersons = async () => {
    ...
    const response = await fetch(
      import.meta.env.VITE_API_URL + "/admin/persons",
      options
    );
  ...
  useEffect(() => {
    if (!sendingMessage && chatIndex) fetchChats();
  ...
  const handleActive = async () => {
    ...
    const response = await fetch(
      import.meta.env.VITE_API_URL + "/admin/reverse-activeness",
      options
    );
    const data = await response.json();
    ...
    <div className="bg-gradient-to-br from-blue-200 to-blue-400 h-screen flex w-full items-center justify-center">
      <div className="bg-gray-50 lg:p-8 p-4 rounded-lg shadow-lg w-4/5 mx-auto h-screen">
        <div className="flex sm:justify-between flex-wrap justify-around mb-4">
          <h1 className="text-3xl font-bold text-center mb-4 lg:mb-6 text-blue-900">
            Chats
          </h1>
          <div className="">
            <button
        ...
        <div className="flex flex-col lg:flex-row lg:gap-4">
          ...
          <div className="lg:w-1/3 w-full border rounded-lg lg:h-[42rem] h-[20rem] overflow-auto">
            <h2 className="w-full p-4 text-lg font-bold sticky top-0 bg-white border-b text-center">
              Chat Persons
            </h2>
            <div className="p-4">
              {persons.length > 0 &&
                persons.map(
                  (person, index) =>
                    person.split("/")[0] !== "admin" && (
                      <div
                        key={index}
                        className="flex items-center mb-2 py-1 px-2 rounded-lg cursor-pointer duration-300 hover:bg-blue-100"
